...
def _spec(idx: int, title: str, text: str = "body", images: list[str] | None = None) -> ChunkSpec:
    return ChunkSpec(
        section_path=[title],
        chunk_title=title,
        chunk_text=text,
...
async def test_ingest_uploader_none_skips_image_resolution() -> None:
    ...
    img = EmbeddedImage(
        image_bytes=b"x", alt_text="", doc_order=0, ext="png", sha256="b" * 64,
    )
    pr = _parser_result(images=[img])
...
async def test_ingest_image_upload_batch_failure_is_non_fatal() -> None:
    ...
    img = EmbeddedImage(
        image_bytes=b"x", alt_text="", doc_order=0, ext="png", sha256="c" * 64,
    )
    pr = _parser_result(images=[img])
    chunks = [_spec(0, "S1", images=["img@0"])]
