...
| `hydration-integrity.spec.ts` | BUG-032 regression — zero React hydration errors on SSR full-load | `/login` + `/register` (auth-frame theme toggle) + `/dashboard` (AppShell `nav/theme-toggle`); uses `colorScheme:'dark'` to surface the `next-themes` SSR mismatch |
