...
- **✅ W49(本期,shipped 2026-06-05)— faithfulness 質素軸抗噪 N-run band(決策 7 Option 1)**
  ...
  - **採用 Option (a) N-run band**(下方候選做法)+ **(c) N=1 warning**;**(d) length-bias 標示** → ✅ **W50 shipped**(平 part:UI explainer affordance — 忠實度 label tooltip + A/B grid 下 caveat note,frontend-only static explainer 無 threshold);**(d) completeness/recall 對沖指標**(ideal 半邊)仍留 W51+ 評估(見證據②)。
