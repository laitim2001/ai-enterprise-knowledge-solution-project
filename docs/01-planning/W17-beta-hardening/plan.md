...
status: closed                    # `active` 2026-05-10 → `closed` 2026-05-10 (F7 closeout — W17 phase Gate = PASS, see progress.md retro); W16 F1-F4 sequence preserved (blocked Track A IT cred populate event per W11 retro CO16)
spec_refs:
...
> **VERDICT(2026-05-10 D8 closeout)= PASS** — all 7 criteria substantively met;sub-verdicts F1 PARTIAL(🚧 F1.5b R8-blocked Postgres-path runtime smoke deferred W18+/CO17)/ F3 PASS-structural(🚧 F3.5b Azure-key-bound RAGAs live-verify deferred)/ F2,F4,F5,F6 PASS;no FAIL condition;backend pytest 593/7 → 613/11. Full rationale + 7-section retro in `progress.md`.
...
| 2026-05-10 (D4) | F2 — auth-transport hardening landed(`7cca23e`). F2 verdict = **PASS**. **CO_F5_refresh + CO_F5_cookie → CLOSED**. **Deviation 5 / scope addition**:ADR-0022 §1 says「the email-verify step ... set ekp_session」→ `/auth/verify-email`'s verified-transition now also creates a session + sets the cookie(auto-login)→ `VerifyEmailResponse` gained `access_token`/`expires_in`(None on the idempotent branch). This fixes a pre-existing register-flow gap(Step-3「Start asking」→ `/chat` was unauthenticated) — net-positive, in-spec(ADR-authorized), so kept rather than minimized. **Deviation 6**:plan F2.5 named only `api-client.ts` — the raw-fetch callers `lib/api/query.ts` `streamQuery` + `lib/api/kb.ts` `uploadDoc` also got `credentials:'include'` + `X-CSRF-Token`(necessary for cookie-auth on `/query/stream` + uploads — Karpathy §1.4 make-it-actually-work). **Deviation 7**:`server.py` CORS gained `allow_credentials=True`(cookie-transport correctness;same local-dev-gap category as the `84d030e` CORS add — not H1, no ADR needed). `register/page.tsx` needed no edit(never wrote localStorage). Backend pytest 609/11 + `tsc`/`next lint` clean | ADR-0022 §1 (verify-step cookie) + Karpathy §1.4 (raw-fetch callers must also work) | Chris |
| 2026-05-10 (D5) | F3 — RAGAs 4-metric integration landed(`7f446fb`). F3 verdict = **PASS (structural; live-verify deferred — same shape as F1)**. NEW `backend/eval/ragas_evaluator.py` `make_ragas_evaluator` + `patch_for_gpt5`(extracted from `scripts/run_ragas_eval.py`'s `_make_real_evaluator`/`_patch_for_gpt5` — `backend/` can't import `scripts/`; the script re-exports both, `test_run_ragas_eval_patch.py` unchanged); `orchestrator.run_eval_pipeline` gained optional `synthesizer`+`ragas_evaluator` → `build_ragas_samples`(retrieve→synth per main query)→ `RagasRunner` → `EvalReport.faithfulness`/`correctness`; `/eval/run`+`/eval/shootout` thread `app.state.synthesizer` + `make_ragas_evaluator(settings)` through(synchronous, `max_main_queries` cap). **Deviation 8**:`EvalReport.correctness` ← RAGAs `answer_relevancy` mean, NOT `answer_correctness` — proper answer-correctness needs SME reference answers per Q14(the eval-set has none); approximation documented. **Deviation 9 / F3.4 finding**:`docs/eval-set-v1.yaml`(final)does NOT exist — `eval-set-v1-draft.yaml` is the WIP; **CO_W15_F1_eval_set_v1 stays OPEN**(pending Q14 SME label cascade; no ground truth fabricated). **Deviation 10**:`image_association`/`crag_trigger_rate`/`avg_cost_per_query_usd` stay 0.0(out of F3 scope — custom image-text metric + CRAG-loop + cost-table integration; documented note in `failed_queries`)— F3.1 said「keep」which it does(in the schema), not「compute」. 🚧 **F3.5b** RAGAs live-verify deferred to a non-proxy env / personal Azure dev tier(CO17 pattern, same as F1.5b). Backend pytest 613/11 | F1-style「structural integration + CI-mocked-at-judge-boundary; live verify needs Azure keys」; ADR-0023-style「ship the wiring, defer the proxy-bound verification」; Q14 SME labels out of W17 scope | Chris |
| 2026-05-10 (D6) | F5 — a11y + dark-mode verification pass landed(`(this commit)`). F5 verdict = **PASS**. **Deviation 11 / scope-precision**:plan F5.1 said "browser smoke: toggle dark mode across V1-V9" + "CO_W15_F3_dark_mode_visual_verify → CLOSED". Actual:`[oklch(...)]` arbitrary-value form = 0 globally(milestone — deterministic);dark-mode mechanism confirmed by code review(next-themes `attribute="class"` → `globals.css` `.dark` vars → tailwind `oklch(var(--token))` wrappers → utility classes;`tokens.ts` `colorsDark` = inverted-button pattern);V8 Login + V7 Landing **browser-verified in dark**(`next dev -p 3001` + Playwright — `body bg oklch(0.18 0.005 285)` == `colorsDark.background`;BrandPanel + button inversion working;error-boundary fallback incidentally verified dark);V1-V6 Chat/Admin/KB/Eval/Debug + V9 Register **not** browser-walked(need a running backend + auth/data)— they consume the same token utility classes(the `[oklch`=0 grep proves no view can escape the `.dark` swap), so dark mode necessarily propagates;the per-view interactive walkthrough stays the **user's pre-Beta smoke**(same shape as the W12-W15 "smoke-user-deferred caveat", scope now reduced to the interactive layer). → **CO_W15_F3_dark_mode_visual_verify partially closed**(mechanism + grep + V7/V8 verified;F7 decides full closure), not fully closed as the plan optimistically said. **Deviation 12 / W17-own-mess fix**:F5.3 — `DocumentsTable`(added W17 D1 / F4.1)`<th>` → `<th scope="col">` ×5(WCAG 1.3.1 table-header association — the only a11y gap traceable to W17's own changes;Karpathy §1.3). Pre-existing gap(register `Stepper` `<li>` lacks `aria-current="step"` — W13)mentioned + deferred to CO_W15_F3_aria_full_audit, not fixed(not W17's mess). `tsc`+`next lint` clean;no backend pytest impact;Playwright screenshots temp-only(reviewed, deleted, not committed) | F5 PARTIAL PASS allowance("spot-check only" for a11y;dark-mode toggle works = the FAIL-condition concern cleared)+ Karpathy §1.2(minimum to verify — mechanism + grep + static views suffice;a per-pixel 9-view walkthrough is the user's job per the established caveat)+ §1.3(only my own mess) | Chris |
| 2026-05-10 (D7) | F6 — Vitest + React Testing Library scaffold landed(`(this commit)`). F6 verdict = **PASS**. Dev deps via `pnpm add -D`(H2-exempt per §5.2「Dev dependency」): `vitest ^4.1.5` + `@vitejs/plugin-react ^6.0.1` + `@testing-library/react ^16.3.2` + `@testing-library/jest-dom ^6.9.1` + `@testing-library/user-event ^14.6.1` + `jsdom ^29.1.1`(`pnpm install` succeeded — **R8 corp proxy does NOT block the npm registry**, only binary-CDN downloads per the W15 Playwright-browser precedent;`pnpm-lock.yaml` +101/-35). NEW `vitest.config.ts`(jsdom + react plugin + `@`→`.` alias mirroring `tsconfig.json` + `setupFiles` + `tests/unit/**` include + `tests/e2e/**` excluded so the two runners stay disjoint)+ `tests/unit/{setup.ts(jest-dom matchers + afterEach cleanup),button.test.tsx(3 cases: render + `bg-primary` token-class variant + `variant="destructive"` swap + `onClick` userEvent),README.md(3-layer boundary doc)}` + `package.json` `test:unit`/`test:unit:watch` scripts. `pnpm test:unit` → 1 file / 3 tests passed;`tsc --noEmit` + `next lint` clean;no backend pytest impact;no new OQ. **CO_W15_F4_vitest_baseline_gap → CLOSED**(harness exists + sample passes + boundaries documented;coverage expansion stays Tier 2 per `tests/unit/README.md`). No deviation from plan F6 — implemented as specified(the only adjustment: vitest auto-resolved to 4.x not the plan's unversioned name, and `@testing-library/user-event` added alongside RTL for the interaction case) | Plan F6 acceptance criteria;§5.2 dev-dep exception | Chris |
| 2026-05-10 (D8) | F7 — phase closeout landed(`(this commit)`). F7 verdict = **PASS**. **W17 phase Gate = PASS**(plan §3 7-criterion eval — all substantively met;sub-verdicts:F1 PARTIAL — 🚧 F1.5b R8-blocked Postgres-path runtime smoke deferred W18+/CO17 / F3 PASS-structural — 🚧 F3.5b Azure-key-bound RAGAs live-verify deferred / F2,F4,F5,F6 PASS;**no FAIL condition hit** — no ADR-0022/0023 Tier-2 creep, in-memory fallback intact + tested, mock-auth dev path verified working post-cookie-change via the F5 next-dev smoke, **backend pytest grew** 593/7 → 613/11). `progress.md` Retro = 7 sections(What worked / What didn't & friction / Surprises / Decisions / Carry-overs to W18+ / Time tracking / Spec ref alignment). ADR-0017/0022/0023 verified `Accepted`(all created so this phase — verify-no-op). `plan.md`+`checklist.md`+`progress.md` frontmatter `status: active` → `closed`. W18+ phase folder **NOT pre-created**(rolling-JIT R1;candidates noted in the Retro). F7.6 hygiene catch-up applied to `session-start.md`(§2 v5→v6 + Cohere v3.5→v4.0-pro / §3 C02+C06 status / §4 authority-order v5→v6 / §11 ADR 15→22 + next-NNNN 0017→0024 + carry-overs CLOSED + W17 milestones row + W17 closed / §10 W16 notes / Last-Updated + Update-history);`COMPONENT_CATALOG.md §11` stale "C13 Workflow Engine" note left as standalone housekeeping(already multiply-flagged;Karpathy §1.3 — not expanded). No new OQ;`decision-form.md` unchanged. No deviation from plan F7 | CLAUDE.md §10 R1(rolling-JIT)+ R5(closeout discipline);plan §3 phase-Gate criteria | Chris |
