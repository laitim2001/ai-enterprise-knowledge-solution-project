...
> **F1 verdict = PARTIAL**(2026-05-10 D2)— code landed `2453a50`,but `pip install psycopg[binary]` is **R8 corp-proxy-blocked**(5th cumulative R8 occurrence → ADR-0017 trigger;vendor-decision pivot point)→ local Postgres-path verification deferred + F1.5 / F1.8 held pending the user decision(see plan §7 changelog D2). Code is shippable regardless(dep declared,lazy-imported,in-memory path unaffected).
...
- [ ] 🚧 F1.5 `backend/api/auth/users_repo.py` Postgres-backed path — **HELD pending the R8 / vendor decision**(plan §7 changelog D2). `users_repo` is module-level functions(not a clean Protocol like `KBStorageBackend`)→ needs a small Protocol refactor;will do once the storage approach is confirmed
...
- [ ] 🚧 F1.8 `docs/architecture.md` §3.4 + `docs/02-architecture/COMPONENT_CATALOG.md` C02 + `docs/setup.md §4.2` — status note in-memory → Postgres-backed + setup table — **HELD pending the R8 / vendor decision**(if storage pivots to sqlite3 the note differs);CO18 → CLOSED only after F1.5 lands
