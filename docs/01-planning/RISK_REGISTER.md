...
| **R14** ★ | **R-B1 Q11 Entra ID tenant operational delay**(NEW W6 D5 / W7 D5;**W8 D5 escalated**)| W6 D5 stakeholder approval cycle decision-level Resolved 2026-05-05;W7 全程 a-revised mock auth dev mode decoupling;operational cascade trigger W8 D1;**W8 D5 closeout 時仍 in-progress past escalation threshold** | 🔴 **High**(Beta-deploy LIVE-blocking;G1+G2+G3+G7 deferred W9 per Chris external) | C11(primary)+ C09 + C10(login flow UI)+ **C12(infra deploy gating)** | 🔴 **Active escalation 2026-05-23**(W8 D5 closeout 觸發 W8 plan §4 R1 escalation trigger threshold):Chris IT engagement(Tenant Access + App Registration + Owner Identification per `beta-plan-v1.md §2 W8.F1`)past W8 D5 仍 in-progress external — escalation 三方:Stakeholder + IT manager + Chris alignment session needed W9 D1。**Mitigation preserved**:W7 a-revised mock auth strategy + W8 D2-D3 real msal_provider wire(backend python-jose + frontend msal-react)+ W8 D4 Chris-cascade SOPs(`infrastructure/entra-id/README.md` + `infrastructure/swa/README.md`)means W9 portal apply session lands implementation spec-complete — IT delivery 是唯一 missing piece。**W8 closeout PARTIAL PASS verdict reflects**:G1' + G4 substitute + G5 + G6 = 4/7 PASS without IT cred(F1.7-mock W7 D5 + W8 D2-D3 unit tests + F4 substitute W8 D4 + F5 observability cascade W8 D5);G1 + G2 + G3 + G7 deferred W9。**Architecture impact zero**(per CLAUDE.md §5.1 H1)— `feature_auth_mock=False` default production gate preserves C11 design intent。**Decay**:R-B1 closes when W9 IT cred delivered + F1.7 LIVE smoke PASS + ACA/SWA/KV apply complete;若 W9 D3 仍未 confirm → Beta phase entry-blocking → W11-W12 production launch milestone risk(Stakeholder cycle re-engaging required) |
