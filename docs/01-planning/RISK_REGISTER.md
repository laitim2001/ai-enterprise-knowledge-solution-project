...
| **R14** ★ | **R-B1 Q11 Entra ID tenant operational delay**(NEW W6 D5 / W7 D5)| W6 D5 stakeholder approval cycle decision-level Resolved 2026-05-05;W7 全程 a-revised mock auth dev mode decoupling;operational cascade trigger W8 D1 | 🟡 Medium(Beta phase entry blocking if W8 D5 仍未 confirm)| C11(primary)+ C09 + C10(login flow UI)| 🟢 **Mitigated 2026-05-16 via W7 a-revised mock auth dev mode strategy**:`Settings.feature_auth_mock=True` + `backend/api/auth/mock_msal.py` + `frontend/lib/auth/mock_msal.ts` decoupled W7 D1-D5 dev work from Q11 IT cred dependency。W7 closeout F1.7-mock substitute(test_f1_7_mock_smoke.py 9 cases)PASS without IT cred。**🟡 Active monitor W8 D1 onward** — Chris IT engagement(Tenant Access + App Registration + Owner Identification per `beta-plan-v1.md §2 W8.F1`);若 W8 D5 仍未 confirm → escalation Stakeholder + IT manager 三方;F1.7 LIVE smoke 推 W8 D4 deploy-time gate(natural per a-revised strategy)。**Architecture impact zero**(per CLAUDE.md §5.1 H1)— `feature_auth_mock=False` default production gate preserves C11 design intent。**Decay**:R-B1 closed 2026-05-22 if W8 D5 IT cred delivered + F1.7 LIVE smoke PASS;else escalates Beta-blocking |
