...
| **R14** ★ | **R-B1 Q11 Entra ID tenant operational delay**(NEW W6 D5 / W7 D5;**W8 D5 escalated;W9 D1 三方 outcome → de-escalated**)| W6 D5 stakeholder approval cycle decision-level Resolved 2026-05-05;W7 全程 a-revised mock auth dev mode decoupling;operational cascade trigger W8 D1;W8 D5 closeout escalation;**W9 D1 三方 alignment session outcome**:Option B-extended(IT 預期 early June 2026 real-calendar deliver)+ Pattern A confirmed + domain `ekp-beta.ricoh.com` confirmed | 🟡 **Medium**(timeline secured;mock auth bridges interim;LIVE deploy推 W11+ natural milestone alignment)| C11(primary)+ C09 + C10(login flow UI)+ C12(infra deploy gating)| 🟡 **Active monitor with confirmed deadline 2026-05-26**(W9 D1 三方 outcome de-escalated 🔴 → 🟡):IT manager committed delivery target **early June 2026 real-calendar(~2026-06-02 to 2026-06-07)**;**Pattern A combined SPA+API confirmed**(NO compliance push for Pattern B);domain `ekp-beta.ricoh.com` confirmed for Beta SWA。**Real-calendar context**(per Chris W9 D1 outcome briefing):real today 2026-05-06 = project doc calendar ~W2 D2,即 EKP team 實作進度 front-run real calendar ~3-4 週,所以 IT 4-week wait 完全 fit project W11-W12 staged rollout milestone window。**Mitigation preserved**:W7 a-revised mock auth strategy + W8 D2-D3 real msal_provider wire(backend python-jose + frontend msal-react)+ W8 D4 Chris-cascade SOPs(`infrastructure/entra-id/README.md` Pattern A 8-step + `infrastructure/swa/README.md` 5-step DNS + `infrastructure/keyvault/README.md` 6 secrets);**W9 D1 alignment memo `docs/03-implementation/r-b1-alignment-memo-2026-05-26.md` was the de-escalation enabler**(Chris pre-session prepared with decision options)。**Re-escalation trigger**:若 real-calendar 2026-06-08 仍未 IT deliver → 🟡 → 🔴 cycle re-engage Stakeholder + IT manager。**Decay**:R-B1 closes 🟢 when IT cred populated to Key Vault + F1.5+F1.7 LIVE smoke PASS + ACA/SWA apply complete(predicted ~project W11)|
