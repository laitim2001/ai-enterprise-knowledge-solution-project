...
**下一步**:**browser end-to-end 驗證**(frontend dev + `NEXT_PUBLIC_AUTH_MOCK=true` mock 登入 + `NODE_TLS_REJECT_UNAUTHORIZED=0`,切 toggle 睇 nav + dashboard 變中文;唔使起 backend)+ 之後逐 view cluster 擴(chat / kb / docs / settings / integrations / users / login / CH-023 7 檔 + Labs 釐清)。
