...
- [ ] F4.2 core flow view externalize(分批:**app-shell ✅ 批 1a** / **dashboard ✅ 批 1b** / **kb-list ✅ 批 2** / **topbar 3 組件 ✅ 批 3**(theme-toggle/notifications-menu/user-menu)/ **users ✅ 批 4**(4 tab + 3 dialog + StatCard;role 專有名 / status badge / TIER 2 / T2 / vendor / ADR / API 路徑保留英文;role-gated 深層視覺待 backend 起驗)/ chat / kb-detail / docs / settings / integrations / **login ✅ 批 5** + **register ✅ 批 6**(login/page.tsx + register/page.tsx 3-step + validation + 3 error handler + AuthFrame login/register 共用 frame;auth-modes / MSAL / build footer / ACS footer / metric 值 / vendor / 範例值 保留英文);**Labs Tier 2 items 待後續** — labels 多技術名 + "Multi-Language" reason F1 promote 後 stale,連 F6 一併釐清)
