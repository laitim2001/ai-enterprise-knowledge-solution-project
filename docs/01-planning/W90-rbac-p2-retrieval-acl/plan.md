...
| 2026-06-24 | **P2.3 用戶 2 決策(AskUserQuestion)**:① **用戶 clearance 來源 = 由 workspace role 推導**(admin = restricted-cleared 見全部;非 admin = internal 只見 internal)。零 schema 改動,符合 plan §5「真實 SSO clearance 來源」out of scope;P3 加真 per-user clearance grant 才細分(如 editor 可見 restricted)。② **文件標記 = 完整閉環 admin-only `PATCH /kb/{kb_id}/docs/{doc_id}/classification`**:即時 merge 更新該 doc chunks 嘅索引 `classification` 欄位(無需 re-ingest)+ 持久化(新 `doc_classification_store`,re-ingest 經 `_run_ingest_pipeline` 讀返 stamp 保留)。| P2.3 kickoff |
| 2026-06-24 | **P2.3 think-before-coding 關鍵 simplification(Karpathy §1.1/§1.2)**:既然 clearance 由 role 推導且**只有 admin = restricted-cleared**,而 `principals_for_user(admin)` 已返回 `None`(完全 bypass filter)→ **「principals 係 None」⟺「admin」⟺「restricted-cleared」**。所以 classification clause **唔需要再 threading 第二個 `user_clearance` param 落 11 層** —— 凡 `_build_acl_filter` 收非 None list(非 admin = internal clearance)就加 `(classification eq 'internal' or classification eq null)`,收 None(admin / 診斷工具 / V4)就乜都唔加。filter 側改動完全 self-contained(零 re-threading,surgical §1.3)。隱式 coupling 嘅失敗模式 = over-restriction(對應 fail-safe),非洩漏 → 可接受 + 強 docstring 標明 P3 per-user clearance 須改 explicit。`null` disjunct = production-preserve fail-open(未重建 KB 嘅 chunk classification null → 視為 internal 可見,同 `allowed_principals` 空集 fail-open 對稱)。| P2.3 think-before |
| 2026-06-24 | **P2.3 標記端點守衛 = `require_role("admin")`**(workspace admin only,對齊用戶「admin-only」字面)。其餘 doc-write 端點(profile override / metadata)用 `require_kb_acl("edit")`,但 classification 係 security 控制,admin-only 係最保守起點;P3 如需 KB manager 可標記 → 放寬至 `require_kb_acl("manage")`。| P2.3 實作 |
