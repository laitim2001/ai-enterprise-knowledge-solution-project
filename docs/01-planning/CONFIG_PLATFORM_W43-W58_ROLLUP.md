...
per-config controlled A/B(DD-4 前後;同一 backend session,只 process env override DD-4 三旋鈕,同 `/query`
path / KB `drive-images-1` / 180s timeout — 唯一變數 = DD-4):

- **A 臂 pre-DD-4** = mean recall **0.572** / precision 0.982 → **重現 W59 baseline 0.5715**(sanity PASS,
  證明 DD-4 是唯一變數,非跨 session confounding)。
- **B 臂 post-DD-4** = mean recall **0.609** / precision 0.977 → **DD-4 淨 +3.7pp**。
- **改善來源 = Q005**(W59 失敗分類 C「section miss」,returned 9→19,recall 0.28→0.56,+0.28)。**實證:
  DD-4 的 `parent_doc` + `citation_expansion`(depth=1)正是解 Q005 section 覆蓋缺口的手段** —— §4.4
  「C 類語義層、配置解不了」由 DD-4 **部分推翻**(section-覆蓋型 C 類,DD-4 這個檢索層配置解到;真語義
  缺失型才解不了)。
- **cap 天花板(Q001/Q036/Q043)+ ≤cap(Q002/Q004/Q006)對 DD-4 0 反應** → 圖片召回**下一槓桿**仍是
  **per-doc 放寬 returned cap**(由實數確認;cap-bound 3 條 returned 卡 20)。
- precision 0.982→0.977 幾乎不變(Q005 多返回 9 張全命中 hit 9→18)。
- **caveat**:single-run A/B;Q005 +0.28(returned 翻倍)遠超 ~20% run variance = robust;Q003 +0.05 弱信號。
...
3. **圖片召回指標(§4.5)** —— ✅ **W59 指標 + baseline + W60 A/B(2026-06-11)**:W59 建指標 harness +
   ...
   補做 DD-4 軸 controlled A/B**:DD-4 淨 +3.7pp(0.572→0.609),精準解 W59 唯一 section-miss 異常(Q005
   returned 9→19),cap 天花板 query 0 反應 → 下一槓桿確認 = per-doc 放寬 returned cap。亦是 Gap B 與
   §4.6 結構性路線的共同判據。**仍未盡**:prose 型第二份 GT(硬依賴用戶提供文件 + 親自標注,延後)+
   cap 放寬軸 A/B(候選)(詳 §4.5 末「W60 A/B 落地」+「未盡部分」)。
