...
| Status | **closed — G-W101 PASS WITH PRE-EXISTING-VITEST-FAIL + LIVE-DEFERRED CAVEAT**(2026-06-30;F1–F6 全落 + F7 收尾;backend ingestion diff=零 + pytest 80 passed,frontend tsc+eslint clean + 5 surface H7;14 vitest fail = pre-existing 既有債 W101 零 regression;live 留 runbook)|
...
| 2026-06-30 | **用戶 approve + F1–F6 落地**:用戶 AskUserQuestion 確認 #2 credential 唯讀 + approve plan → active。F1 backend 端點(`9f2818e`)/ F2 nav(`d5dcfc7`)/ F3+F4 API client+landing(`5611f2e`)/ F5 wizard step1+2(`9972a18`)/ F6 wizard step3+4(`7545bde`),逐 deliverable tsc+eslint / pytest+ruff+mypy 綠。**實作期再揭 2 處張力**(用戶 AskUserQuestion 各拍板):**#step3 streaming**(mockup per-doc streaming vs backend 同步 import → 揀 **A 整體 progress 過場**,plan §4 記)+ **#step4 mini-stat**(mockup「Chunks added」vs backend summary 無 chunk count → 改「Documents」,H7 minor deviation)。 | F1–F6 |
| 2026-06-30 | **F7.1 deviation(R3)**:plan §2 F7.1 原寫「upload `SourceKind='sharepoint'` placeholder 改指向 `/integrations/sharepoint/import`」,但發現該 placeholder 屬 **upload page 自己 mockup(`ekp-page-misc.jsx`)一部分**(disabled「Wave C+」URL input),改之**偏離 upload mockup 觸 H7** → STOP+ask → 用戶揀 **不改 upload**(保留 placeholder,維持 upload mockup fidelity;disabled non-functional 唔構成真 drift)。F7.2 component test **🚧 deferred**(per DD-2,階段 1b 純前端 UI,H6 UI test nice-to-have)。 | F7.1 |
| 2026-06-30 | **F7 收尾 + Phase close — G-W101 PASS(caveat)**:F7.3 Gate / F7.4 doc-sync。**Gate**:backend ingestion diff **= 零**(§7.2)+ pytest **80 passed**(integration 24 + RBAC 21 + 其他)+ ruff/mypy clean;frontend tsc+eslint clean + 5 surface H7 對齊 mockup(inline-style verbatim 值)。**vitest 14 fail = pre-existing**(`chat-meta-row`/`kb-detail`/`kb-settings-reindex`/`kb-settings-tuning` kb config view test render error;`git diff --stat babccd8 HEAD -- frontend` 證 W101 **只**改 `app-shell`(+11)+ 3 integrations 新檔,**零 touch** 嗰啲 view code → 14 fail W101 前後逐字節一致 = **0 regression**;單跑 4 file 穩定 14 fail/9 pass = 真 pre-existing broken 非 flaky;`app-shell.test` **pass**)。**Live D1-D4 留 runbook**(真 tenant + `Sites.Selected`,D4)。**14 pre-existing frontend test 債入 BACKLOG E**(W101 外,Karpathy surgical 只 mention 唔修)。 | F7 + close |
