...
**Next**:重啟全服務(用戶 trigger,backend running pre-F2 → `/profile-presets` 404 須載入 F2)→ F4 browser
驗 GET/PUT/DELETE round-trip。
...
- (F3)feat(frontend): W82 F3 settings preset-mapping edit UI (ADR-0063)
