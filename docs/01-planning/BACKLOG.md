...
| **B-25** | 雙語 i18n(en/zh UI 切換)— Multi-language Tier 2 promote(dictionary + locale state + 全站 externalize + 切換 UI + next-intl) | `進行中(W103 主體完成,剩用戶校對 + retro)` | **[ADR-0075](../adr/0075-multi-language-i18n-tier2-promote.md) Accepted(2026-07-14)雙 approve**;**W103 主體 2026-07-21 全落地**:F1 Tier 邊界 amendment / F2 next-intl 4.13.2(H2)/ F3 cookie-based locale(D-2 甲,URL 不變)/ F4 全站 externalize(**37 namespace / 1588 keys**,en 定稿 + zh draft)/ **F5.2 GLOSSARY.md 術語權威**(4 項用戶拍板 + 批次統一 54 條)/ F6 language toggle 正式化(mockup PopMenu + 登入頁 + Settings)/ F7 H7 sub-gate(en 零回歸 + zh 零爆版 + CJK font 四層同步 + i18n 守門測試 16 條 + build 17 route clean)/ F8.1-F8.4 doc-sync(ADR impl note + GLOSSARY §F 維護規則 + CLAUDE.md routing + user-guide 01 §6)。**剩:F5.3 zh 1588 條用戶校對(卡用戶)+ F8.5 closeout retro** → 兩者完成先改「完成」 | [ADR-0075](../adr/0075-multi-language-i18n-tier2-promote.md) / [W103 phase](./W103-i18n-en-zh/plan.md) / [GLOSSARY.md](../../frontend/messages/GLOSSARY.md) / architecture.md §11 / CLAUDE.md §5.4 H4 |
