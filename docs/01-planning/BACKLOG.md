...
| **B-25** | 雙語 i18n(en/zh UI 切換)— Multi-language Tier 2 promote 評估(建 dictionary + locale state + 全站 UI string externalize + 切換 UI + 新 i18n dependency 如 next-intl) | `ADR Proposed`(等雙 approve) | **[ADR-0075](../adr/0075-multi-language-i18n-tier2-promote.md) drafted(Proposed,2026-07-13)**:用戶確認 zh/APAC driver → draft promote 評估(scope 限 **UI chrome 雙語**,非 RAG content/JP);**待 Chris(架構 H1/H2)+ Stakeholder(scope H4 via `decision-form.md`)雙 approve 先實作**,approve 後開 multi-day phase(§10 R1)+ H7 sub-gate。最大成本 = zh 翻譯來源 + 雙語同步維護 | [ADR-0075](../adr/0075-multi-language-i18n-tier2-promote.md) / architecture.md §11 / CLAUDE.md §5.4 H4 / [CH-023 §6](../03-implementation/changes/CH-023-unify-frontend-ui-language-english/spec.md) |
