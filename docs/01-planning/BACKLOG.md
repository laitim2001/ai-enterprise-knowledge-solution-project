...
| **B-22** | `aiohttp` 未在 `backend/pyproject.toml` base deps 宣告 — `ingestion/screenshots/uploader.py` + `integration/source_store.py` 的 `azure.storage.blob.aio` runtime 真需要;現只靠 `eval` extras(instructor/langchain)transitive 意外滿足 → fresh production deploy 只裝 `.[dev]` 會 runtime 爆 "Unable to create async transport"(2026-07-08 B-20 CI-green 時實證) | **與 B-23 綁定**:直接加 aiohttp 會令 CI full `pytest -q` hang(見 B-23）→ 必先修 B-23 才可加;加時同步 declare + 驗 CI 仍綠 |
| **B-23** | full `pytest -q` 有 **pre-existing 網絡測試會 hang**(CI 無 Azurite 時觸真 async blob / 網絡連線);現靠「CI 無 aiohttp → 相關測試 fast-fail」意外避開(W100 整合 Gate 曾因此改用「針對性 Gate」為證) | 找出 hang 測試(疑 `tests/api/` 觸真 blob 未被現有 stub 蓋齊;本地 Azurite 會遮蔽故需無 Azurite 環境重現)→ 正確 mock/skip;修好才可解 **B-22**。2026-07-08 加 aiohttp 實測撞到此 hang(卡 4% ~12 分鐘)為證 |
