...
| Status | **closed — G-W100 PASS WITH FULL-SUITE-ENV + LIVE-DEFERRED CAVEAT**(2026-06-30;F1–F6 全完成,49 新測試 + RBAC 回歸綠,ingestion 核心零改動;前端 wizard = 階段 1b carry-over,live 驗證留 runbook)|
...
| 2026-06-30 | **F2–F6 全完成 → G-W100 PASS,phase closed**:F2 Graph REST client(`abfaf1e`,7 test)/ F3 connect·browse·list·fetch(`bfa1d34`,9 test)/ F4 get_principals 權限映射(`0499f2d`,12 test)/ F5 import service(`08ce773`,6 test)/ F6 API route + production adapter(本 commit,8 test)。**49 新測試 + RBAC `test_kb_route_acl` 21 回歸綠**;ruff/`mypy --strict`(integration package + route module)clean;**`backend/ingestion/` git diff = 零**(§7.2 鐵律)。**Gate caveat**:full repo pytest hang 喺 pre-existing 網絡綁定測試(非本 phase;hang 前 ~216 全綠)→ 針對性 Gate。**carry-over**:前端 wizard = 階段 1b(用戶選);live 驗證留 runbook §10(D4);org/public/external_group principal 端到端 = follow-up。**無新 ADR / 無新 vendor**(H2 確認無新 dep)。 | F2–F6 + close |
