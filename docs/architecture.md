...
> **W50 amendment per 決策 7 Option (d)(length-bias affordance — 平 part)**(F1-F2,**frontend-only**,H7 design-first):收 W49 known limitation 嘅教育半邊。config-test 試跑 panel 加 **length-bias explainer affordance**:(a)「忠實度」headline label `title=` tooltip(faithfulness 性質 + length bias 警示);(b) A/B 結果 grid 下方單一 **caveat note**(`AlertTriangle` warning 色 + 「忠實度對長/全面答案有 **length bias** —— 低分若配合高引用數/長答案,多為 bias 而非 config 差,宜對照引用數/字數(+ 將來 recall)判讀,勿與完整性混為一談」)。**static explainer 無 arbitrary threshold**;用 result card 現有資料(無 backend / 無新 metric / 無新 dep / 無新 ADR)。**completeness/recall 對沖指標**(roadmap Option d「ideal」半邊)留 W51+ 評估。
