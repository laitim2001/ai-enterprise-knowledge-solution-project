...
| **Stale uvicorn PID 殘留**:backend regression(如 `langfuse_trace_list.py` Langfuse cap clamp、`/health` schema 改 etc.)survived through 整個 phase 唔 detect,因為 dev server 冇 hot-reload + 用 stale process | dev server 唔加 `--reload` flag,或者 backend file edits 落咗 stale PID 範圍外(e.g. 用 system Python 啟動,而後續 edits 透過 `.venv/Scripts/python.exe` ChangeSet 觸發但 stale PID 唔 watch)| **Dev default 必須加 `--reload --reload-dir backend/`**(per §4.3 invocation);**production / debug timing-sensitive 場景 才 opt-out**;detect stale PID via `tasklist /fi "imagename eq python.exe"`(Windows)或 `ps aux \| grep uvicorn`(Unix);kill safely via `Stop-Process -Id <PID>`(Windows)或 `kill <PID>`(Unix)。CLAUDE.md §4.4 hard rule:**唔好用 `--no-verify` 跳過 git hook 嘗試「workaround」regression**(per W22 D8 surfaced + W23 F4 amendment).實際 example:W21 F2 commit `55f876b` Langfuse cap clamp 500 仍 in code,W22 D8 backend regression(Langfuse 400 error)透過 stale PID 37036(2026-05-16 started without `--reload`)survived through entire W22 phase,直至 W22 D8 user-eye `/traces/[traceId]` verify catch + new PID 2092 via `.venv/Scripts/python.exe` 才 expose |

---

### 8.7 Test infrastructure(W23 F4 NEW)

W23 D1+D2 揭露 OneDrive-synced repo 環境下嘅 test 框架 quirks:

| 症狀 | 原因 | 解決 |
|---|---|---|
| Vitest `pnpm test:unit` full suite 部分 file timeout 60s+ + 報「[vitest-pool-runner]: Timeout waiting for worker to respond」 | OneDrive filesystem sync hooks slow forks-pool worker process startup(Windows-specific)| `vitest.config.ts` 已 default `pool: 'threads'`(W23 F1.5 fix)— threads pool reuses `worker_threads` in-process,sidesteps OneDrive process-creation latency。Individual files run reliably under threads;rare full-suite worker timeout(1-2 file out of 13)benign(test logic not failing,just worker startup race)。CLI override:`pnpm exec vitest run <file> --pool=threads`;**唔好 fallback 用 forks pool 喺 OneDrive 環境**(per W23 D1.2 finding) |
| Playwright `pnpm test:e2e` 多個 first-route tests timeout 30s `(page.goto: Test timeout)` | Next.js dev server first-route compile 30-40s per cold route(Windows OneDrive filesystem sync delay + dev mode route-on-demand build) | `playwright.config.ts` 已 `timeout: 60_000`(W23 F2 fix)— 60s preserves CI Beta hardening signal while allowing dev cold-start。CI / Beta hardening invocation **必須用 production build**(`pnpm build && pnpm start`)而非 dev mode — dev-mode E2E係 W23 PARTIAL gate ranging(per W23 D2.4 finding) |
| `PW_CHANNEL=chrome pnpm test:e2e` 個別 spec timeout / element not found 仍出現 | `npx playwright install chromium` R8 corp-proxy CDN-blocked(per W17 ADR-0017 Plan B (a))→ 用 system Chrome 替代,但 system Chrome version 偶爾 drift(corp-managed auto-update) | 跟 ADR-0017 Decision-rule #5 ordered Plan B sequence:**(a)** existing system binary(`PW_CHANNEL=chrome` or `msedge`)→ **(b)** native package manager global install → **(c)** defer to non-proxy env(personal mobile hotspot)。若 system Chrome auto-update 引起 spec drift,先 confirm Chrome version via `chrome://version`,然後 evaluate `PW_CHANNEL=msedge` fallback。CI default unchanged(unset → bundled Chromium) |
| Visual baseline pixel diff false positive(rebuild typography 嘅 oklch token resolve 出微小 color drift) | rare anti-aliasing / sub-pixel rendering jitter 喺 Windows + system Chrome rendering pipeline | Playwright config 已 `expect.toHaveScreenshot.maxDiffPixelRatio: 0.01`(1% 容差)— 個別 spec 過嚴可 per-test override `maxDiffPixels: 200-500`;若 systemic drift → defer 入 W23+ 或 W24 dark-mode token resolve session(W23 F2 不 trigger 此 issue,但 future Wave C+ 可能要 tune)|

W23 D1+D2 經驗:Test infrastructure 喺 OneDrive 環境 + Windows + Next.js dev mode 有 inherent cold-start latency。預 Tier 1 Beta hardening 階段(W16 F1-F4 Track A IT cred 之後),production-build E2E run 應該 < 10s per route + Vitest forks pool 都 stable(production server warm + Docker volumes mount 替代 OneDrive)。
