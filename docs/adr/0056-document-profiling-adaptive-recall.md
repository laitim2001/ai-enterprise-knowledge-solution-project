...
| **P4_scan_imgdense** | 掃描 / 純圖無 heading | PDF **text-layer 空比例高** + OCR-text-ratio | 純 gallery + OCR 依賴(**pending — 0 樣本驗證**)|
...
- **每頁文字密度(chars/page)** — 分 PDF 簡報 vs PDF 文件;需 parser 出 page metadata(目前
  `ParserResult` 無,要加)。
- **PDF text-layer 空比例** — 分 text-PDF vs scan-PDF(P4);用 `pypdfium2`(Docling 自帶,零新 dep)。
...
- **P4 scan 類別 — `Open`**:sample 0 樣本驗證,待用戶補真掃描 PDF(text-layer 空、要 OCR)再
  補驗 + 補一節。
...
- **OQ-A(P4 scan)**:需用戶補真掃描 PDF 驗證 P4 + OCR-text-ratio 信號可行性。
...
- 實證 script:`scripts/explore_doc_profiling.py`(v1)/ `explore_doc_profiling_v2.py`(v2,90%)/
  `explore_doc_content.py`(ground truth + PDF text-layer)
- Sample corpus:`docs/06-reference/01-sample-doc/`(33 份真實企業文件)
