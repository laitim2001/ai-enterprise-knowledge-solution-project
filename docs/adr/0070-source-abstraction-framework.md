...
**Date**: 2026-06-28
**Status**: Proposed(等 Chris 行使 decision owner 拍板 Accept;**Proposed → Accepted 前提 = 3 個 scope decision DG-INT-1/2/3 resolved**,見 Decision §尾)
**Approver**: Chris(decision owner — pending)
...
1. **`SourceConnector` 抽象 + capability model**(H1):定義 provider-agnostic Protocol —— `connect` / `browse` / `list_documents` / `fetch_document` / `get_principals`(已展平 nested group)+ optional `delta`(由 capability gate)。`ConnectorCapabilities` 宣告 `auth_kind` / `supports_browse` / `supports_acl` / `supports_delta` / `acl_granularity`;UI + lifecycle 按 capability 退化(無 delta → 隱藏自動同步;`acl_granularity=none` → 退化 KB 層權限)。
...
5. **nested group 展平 + 防爆量**:push-model 下,connector 自行用 Graph `transitiveMembers` 端點(**非**被反證嘅 `transitiveMemberOf`)把 nested group 展成 flat principal set 填 `allowed_principals`;展開設上限保護(< 2,049 / file,避免 group 爆量)。特殊 principal(Anyone link 無可解析 GUID)按明確規則處理(drop / 當 public / 拒絕索引)。
...
### 待拍板 scope decisions(Proposed → Accepted 前提)

| # | 問題 | 提議 default |
|---|---|---|
| **DG-INT-1** | 第一個 concrete connector = SharePoint? | SharePoint(公司主力)— 另一選項:先做最簡單嘅無 ACL 來源(純 share folder)降複雜度 |
| **DG-INT-2** | 階段 1 範圍鎖死 = SharePoint 按需手動匯入,排除 auto-sync + 多 provider? | 鎖死(守 H4 Tier 邊界,避免滑入 Tier 2) |
| **DG-INT-3** | `SourceConnector` interface + capability model 定案(per Decision 1)? | 按本 ADR 草案;細節入階段 1 plan |
