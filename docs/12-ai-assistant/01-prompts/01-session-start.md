...
| **W21** | **Frontend Wave B**(Doc Detail 3-pane + Eval Console refactor + Traces list + Trace 3 viz)— `/kb/[id]/docs/[docId]` 3-pane per ADR-0029 Option C(outline + chunks + inspector with embedding vector preview Wave B verify)+ `/eval` 6-section refactor consuming W17 F3 RAGAs(NO new backend)+ `/traces` index list view + `/traces/[traceId]` 3 viz modes per ADR-0030 absorbed(vertical default + waterfall + flame SVG)+ 2 NEW backend endpoints(`GET /kb/{kb_id}/docs/{doc_id}` enriched per ADR-0029 + `GET /traces?filter=...&since=...` list per ADR-0030 absorbed) | 🟡 **active 2026-05-17**(kickoff cascade landed per Chris directive「開 W21-frontend-wave-b kickoff」;ADR-0029 Accepted Option C + ADR-0030 SKIPPED-absorbed both already W19 F6 `6a34a41`;`architecture.md v6 §5` 4 inline-tagged amendments(§5.5.2 KB Detail Chunks tab unchanged annotation + cross-ref / NEW §5.5.2a Document Detail page 3-pane per ADR-0029 / §5.6 Eval Console 6-section refactor / §5.7 Traces /traces index + 3 viz modes per ADR-0030 absorbed)landed at kickoff;real-calendar collapse pattern 1.8-12× — actual ~1-2 days vs ~9 plan-day budget;`end_date` ~2026-05-24 window;**Rule-of-3 wizard primitive promotion DEFER**(Wave B no wizard surface per Karpathy §1.3 surgical);**Embedding vector preview feasibility verify at F3.5 implementation time**)|
| W22+ | _not pre-created_ — rolling JIT per CLAUDE.md §10 R1 | (kickoff post-W21 closeout decision;**candidates**:**W22-frontend-wave-c1 + W22b-frontend-wave-c2 SPLIT per W19 F4 §3.6 trigger**(Wave C scope decision at W22 kickoff post-W21 close;ADR-0026 Option B + ADR-0027 Option A combined ~42 backend days exceeds single phase budget;C1 + C2 scope concrete split at kickoff;activates `/kb/[id]` Access tab per ADR-0025;ships `/settings` 6-tab per ADR-0026 + `/users` Tier 1.5 RBAC per ADR-0027 + real-MSAL feature-flagged concurrent ship per user 岔口 2;**2 NEW dependencies** Key Vault SDK + Entra Graph SDK Plan B sequencing decision per ADR-0017)/ **Rule-of-3 wizard primitive promotion**(shared `<Stepper>` + Field/Stage extract per W20 F6.2 trigger — 4th wizard usage hit;Wave C has wizard surfaces /settings + /users — natural fit)/ Wave D Tier 2 hold post-Beta Q12 / W16 F1-F4 Track A IT cred remains parallel track,mock-auth default continues through Wave C per user 岔口 2)|
...
| **W21-frontend-wave-b** | _active 2026-05-17_(kickoff cascade landed)| **W21 kickoff `(this commit)`** + F1-F7 + F8 closeout(pending)| **🟡 active** — Frontend Wave B(Doc Detail 3-pane per ADR-0029 Option C + Eval Console refactor + Traces list + Trace 3 viz per ADR-0030 absorbed;backend ~2 days + frontend ~6.5-7.5 days;real-calendar collapse ~1-2 actual days vs ~9 plan-day budget;window ~1 week ending ~2026-05-24). Kickoff cascade landed:ADR-0029 Accepted Option C verified(W19 F6 `6a34a41` already accepted)+ ADR-0030 SKIPPED-absorbed footnote preserved per W19 F6 closeout + `architecture.md v6 §5` 4 inline-tagged amendments(§5.5.2 KB Detail Chunks tab unchanged annotation + cross-ref / **NEW §5.5.2a** Document Detail page 3-pane per ADR-0029 / §5.6 Eval Console 6-section refactor per Wave B / §5.7 Traces `/traces` index + 3 viz modes per ADR-0030 absorbed)+ W21 `plan.md` + `checklist.md` + `progress.md` created `status: active`. F1-F8 mapped onto ADR-0029 + ADR-0030 absorbed scope + `<DisabledAffordance>` shared component(W20 F1.5 landed)consumed for embedding-vector + Ops Metrics + CRAG fallbacks. **Rule-of-3 wizard primitive promotion DEFER**(Wave B has no wizard surface per Karpathy §1.3 surgical). |
| W22+ | _not pre-created_ | _pending W21 closeout decision_ | (rolling JIT — W22+ phase folder NOT pre-created per CLAUDE.md §10 R1;candidates per W19 F4 Wave breakdown = **W22-frontend-wave-c1 + W22b-frontend-wave-c2 SPLIT per F4 §3.6 trigger**(Option A+B Wave C scope ~42 days exceeds single phase;Settings 6-tab per ADR-0026 Option B + /users Tier 1.5 RBAC per ADR-0027 Option A + Access tab activate per ADR-0025 + real-MSAL feature-flagged concurrent ship;**2 NEW deps** Key Vault SDK + Entra Graph SDK Plan B sequencing decision per ADR-0017;split decision at W22 kickoff post-W21 close)/ **Rule-of-3 wizard primitive promotion**(shared `<Stepper>` extract per W20 F6.2 trigger — Wave C has /settings + /users wizard surfaces — natural fit)/ Wave D Tier 2 hold post-Beta Q12;W16 F1-F4 Track A IT cred parallel track,mock-auth default continues through Wave C per user 岔口 2)|

**累計**:**19 phase closed + 1 active**(W1-W15 + W17 + W18 + W19 + W20 closed;**W21-frontend-wave-b active 2026-05-17** — kickoff cascade landed,F1-F8 implementation in flight;Tier 1 12-week sprint extended via W12-W15 UI sprint cycle pivot — architecture v5.1 → v6 amendment + 9 views — then W17 Beta-hardening parallel to W16, then W18 unified application shell IA per ADR-0024, then **W19-frontend-audit-and-adr-draft** plan phase for design-mockups → real-`frontend/` implementation with 6 ADRs Accepted(0025/0026/0027/0028/0029/0031), then **W20-frontend-wave-a** shipped Wave A `/dashboard` + `/chat` advanced + `/kb` cluster 7-tab + auth/topbar polish via ADR-0025/0028/0031 + ADR-0030/0032 absorbed scope, then **W21-frontend-wave-b** kickoff per Chris directive「開 W21-frontend-wave-b kickoff」implements ADR-0029 Option C `/kb/[id]/docs/[docId]` 3-pane + ADR-0030 absorbed Trace 3 viz + /traces list);**W21 implementation Day-N entries 將 daily append per CLAUDE.md §10 R2**;**W16 status:draft**(F5 backend stub closure cascade DONE;F1-F4 Track-A-blocked pending IT cred populate event + R-B1 closure — parallel-track to W17/W18/W19/W20/W21)
