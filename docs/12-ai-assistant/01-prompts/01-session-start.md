...
| **W20** | **Frontend Wave A**(Dashboard + Chat advanced + KB cluster 7-tab + auth/topbar polish)— `/dashboard` real cards per ADR-0030 absorbed + `/chat` advanced surfaces per ADR-0031 Option B server-side Conversation History(promotes C10 §7 Tier 2 → Tier 1;Postgres `conversations` + `messages` tables per ADR-0023 backing pattern + 6 NEW `/conversations` CRUD endpoints)+ `/kb` list polish(grid+table+filter)+ `/kb/new` 5-step wizard per ADR-0028(Multimodal Tier 1 active + Tier 2 disabled affordances)+ `/kb/[id]` **7-tab `-Access`** refactor per ADR-0025(Documents+Chunks+Images NEW+Chunking Lab NEW+Pipeline+Retrieval Testing+Settings;Access tab disabled affordance Tier 1.5 — Wave C1 activates per ADR-0027 Option A RBAC infra)+ `/kb-upload/[id]` polish + `/login`+`/register` Brand panel + Forgot password disabled + Topbar NotificationsMenu + Workspace switcher disabled(W19 §2.3 Tier 2 leak fix)+ Sidebar Tools sub-section + Labs hidden(F5.4 Option C)+ `<DisabledAffordance>` shared component(W19 F5 spec)+ 9 backend items + Vitest 20+ tests + Playwright route updates | 🟡 **active 2026-05-16**(kickoff cascade landed per Chris AskUserQuestion A1 pick「W20-wave-a 7-tab kickoff」;6 ADRs already Accepted W19 F6;`architecture.md v6 §5` inline-tagged amendments §5.2/§5.3/§5.4/§5.5/§5.5.3/§5.10 landed at kickoff;real-calendar collapse pattern 1.8-4× — actual ~3-5 days vs ~12 plan-day budget;`end_date` ~2026-05-30 window)|
| W21+ | _not pre-created_ — rolling JIT per CLAUDE.md §10 R1 | (kickoff post-W20 closeout decision;**candidates**:W21-frontend-wave-b(`/doc-detail` 3-pane `/kb/[id]/docs/[docId]` per ADR-0029 Option C + `/eval` Eval Console refactor + `/traces` index + `/traces/[traceId]` 3 viz modes per ADR-0030 absorb;backend ~2 days + frontend ~6.5-7.5 days;window ~1 week)/ **W22-frontend-wave-c1 + W22b-frontend-wave-c2 SPLIT per F4 §3.6 trigger**(Wave C scope decision at W22 kickoff post-W20 close;ADR-0026 Option B + ADR-0027 Option A combined ~42 backend days exceeds single phase budget;C1 + C2 scope concrete split at kickoff;activates `/kb/[id]` Access tab per ADR-0025;ships `/settings` 6-tab per ADR-0026 + `/users` Tier 1.5 RBAC per ADR-0027 + real-MSAL feature-flagged concurrent ship per user 岔口 2;**2 NEW dependencies** Key Vault SDK + Entra Graph SDK Plan B sequencing decision per ADR-0017)/ Wave D Tier 2 hold post-Beta Q12;W16 F1-F4 Track A IT cred remains parallel track,mock-auth default continues through Wave C per user 岔口 2)|
...
| **W20-frontend-wave-a** | _active 2026-05-16_(kickoff cascade landed)| `(W19 closeout 6a34a41)` + **W20 kickoff `(this commit)`** + F1-F8 + F9 closeout(pending)| **🟡 active** — Frontend Wave A(7-tab `-Access` scope per A1 pick;backend ~8-10 days + frontend ~10-13 days;real-calendar collapse ~3-5 actual days vs ~12 plan-day budget;window ~1.5-2 weeks ending ~2026-05-30). Kickoff cascade landed:6 ADRs Accepted Status verified(W19 F6 `6a34a41`)+ `architecture.md v6 §5` inline-tagged amendments(§5.2 Chat ADR-0031 server-side Conversation History + advanced surfaces / §5.3 Dashboard ADR-0030 absorbed richer cards + per-component health / §5.4 KB List grid+table+filter polish / §5.5 KB Detail 5-tab → 7-tab refactor per ADR-0025 + Access tab Wave C1 deferral / §5.5.3 `/kb/new` 5-step wizard per ADR-0028 + Multimodal Tier 1+2 / §5.10 Login Brand panel + Forgot password disabled affordance per W20 F7)+ W20 `plan.md` + `checklist.md` + `progress.md` created `status: active`. F1-F9 mapped onto ADR-0025/0028/0031 + ADR-0030/0032 absorbed scope + `<DisabledAffordance>` shared component spec(W19 F5)consumed across Wave A surfaces. |
| W21+ | _not pre-created_ | _pending W20 closeout decision_ | (rolling JIT — W21+ phase folder NOT pre-created per CLAUDE.md §10 R1;candidates per W19 F4 Wave breakdown = W21-frontend-wave-b(Doc Detail 3-pane per ADR-0029 Option C `/kb/[id]/docs/[docId]` + Eval Console + Traces index + Traces 3 viz)/ **W22-frontend-wave-c1 + W22b-frontend-wave-c2 SPLIT per F4 §3.6 trigger**(Option A+B Wave C scope ~42 days exceeds single phase;Settings 6-tab + /users Tier 1.5 RBAC + Access tab activate + real-MSAL feature-flagged concurrent ship;**2 NEW deps** Key Vault SDK + Entra Graph SDK Plan B sequencing decision per ADR-0017;split decision at W22 kickoff post-W20 close)/ Wave D Tier 2 hold post-Beta Q12;W16 F1-F4 Track A IT cred parallel track,mock-auth default continues through Wave C per user 岔口 2)|

**累計**:**18 phase closed + 1 active**(W1-W15 + W17 + W18 + W19 closed;**W20-frontend-wave-a active 2026-05-16** — kickoff cascade landed,F1-F9 implementation in flight;Tier 1 12-week sprint extended via W12-W15 UI sprint cycle pivot — architecture v5.1 → v6 amendment + 9 views — then W17 Beta-hardening parallel to W16, then W18 unified application shell IA per ADR-0024, then **W19-frontend-audit-and-adr-draft** plan phase for design-mockups → real-`frontend/` implementation with 6 ADRs Accepted(0025/0026/0027/0028/0029/0031)+ Wave C C1+C2 split trigger activated per Chris Option A+B picks,then **W20-frontend-wave-a** kickoff per A1 pick「7-tab `-Access`」)— **W20 implementation Day-N entries 將 daily append per CLAUDE.md §10 R2**;**W16 status:draft**(F5 backend stub closure cascade DONE;F1-F4 Track-A-blocked pending IT cred populate event + R-B1 closure — parallel-track to W17/W18/W19/W20)
...
**Last Updated**:2026-05-16(**W20-frontend-wave-a phase kickoff cascade** — Chris AskUserQuestion A1 pick「W20-wave-a 7-tab kickoff (Recommended)」at W19-closeout decision session;per CLAUDE.md §10 R1 rolling JIT + R5 ADR-before-implement discipline。**Kickoff cascade landed**:`docs/01-planning/W20-frontend-wave-a/{plan,checklist,progress}.md` created `status: active`(plan §2 F0-F9 deliverables mapped onto ADR-0025/0028/0031 + ADR-0030/0032 absorbed scope;§5 success criteria + §6 carry-overs from W19/W18/W17/W16 + §7 changelog Day-0 row);`docs/architecture.md` 6 inline-tagged §5 amendments(§5.2 Chat ADR-0031 server-side Conversation History + advanced surfaces / §5.3 Dashboard ADR-0030 absorbed richer cards + per-component health / §5.4 KB List grid+table+filter polish / §5.5 KB Detail 5-tab → 7-tab refactor per ADR-0025 + Access tab Wave C1 deferral / §5.5.3 `/kb/new` 5-step wizard per ADR-0028 + Multimodal Tier 1+2 / §5.10 Login Brand panel + Forgot password disabled affordance per W20 F7)— doc version held per W18 ADR-0024 + §3.4/§3.7 precedent。**6 ADRs Status verified Accepted**(no flip needed — W19 F6 `6a34a41` already accepted)。**§10 W20+ row split** → W20 active row + W21+ not-pre-created candidates row;§12 milestones table 加 W20-frontend-wave-a active row + 累計 18 phase closed + 1 active note。**Backend foundation scope confirmed**:9 backend items per W19 F2 §3.1+§3.2(items 1+2+5+6+7+8 + ADR-0031 Option B items 9+10;items 3+4 Q6/Eval-cache defer empty-state CTA per W18 F4 acceptance)— ~8-10 backend days + ~10-13 frontend days = ~12 plan-day budget,real-calendar collapse pattern 1.8-4× → actual ~3-5 days vs ~12-day plan window ending ~2026-05-30。**Tier 2 boundary enforcement via `<DisabledAffordance>` shared component**(W19 F5 spec)across Workspace switcher + Access tab + Multimodal Tier 2 + Labs hidden + Forgot password + Chunking Lab "Apply"。**Mock-auth default continues through Wave C** per user 岔口 2 — real-MSAL feature-flagged concurrent ship deferred Wave C。**Wave C MUST split into C1+C2** per F4 §3.6 — W22 kickoff scope decision)
