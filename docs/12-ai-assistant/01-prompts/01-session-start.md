...
| W20+ | _not pre-created_ — rolling JIT per CLAUDE.md §10 R1 | (kickoff post-W19 closeout per F4 + 6 Accepted ADRs;**candidates**:W20-frontend-wave-a(`/dashboard` + `/chat` advanced + `/kb` + `/kb/[id]` 7 OR 8-tab + `/kb-upload` + auth polish + topbar polish + ADR-0030/0032 absorbed scope;Wave A scope re-confirm at kickoff:`-Access` 7-tab vs `+Access` 8-tab depends on backend RBAC infra phase split per ADR-0027 Option A acceptance;Wave A backend ~8-10 days + frontend ~10-13 days;window ~1.5-2 weeks)/ W21-frontend-wave-b(`/doc-detail` 3-pane `/kb/[id]/docs/[docId]` + `/eval` + `/traces/[traceId]` 3 viz modes;parallel to W20 if capacity supports;backend ~2 days + frontend ~6.5-7.5 days;window ~1 week)/ **W22-frontend-wave-c1 + W22b-frontend-wave-c2 SPLIT per F4 §3.6 trigger**(Wave C scope decision at W22 kickoff post-W20 close;Option A+B combined ~42 backend days exceeds single phase budget;C1 + C2 scope concrete split at kickoff)/ Wave D Tier 2 hold post-Beta Q12;W16 F1-F4 Track A IT cred remains parallel track,mock-auth default continues through Wave C per user 岔口 2)|
...
| W20+ | _not pre-created_ | _pending W19 closeout decision_ | (rolling JIT — W20+ phase folder NOT pre-created per CLAUDE.md §10 R1;candidates per W19 F4 Wave breakdown = W20-frontend-wave-a(Dashboard + Chat advanced + KB cluster 7-tab or 8-tab + Multimodal + auth polish + topbar polish + ADR-0030/0032 absorbed)/ W21-frontend-wave-b(Doc Detail 3-pane + Eval + Traces 3 viz)/ **W22-frontend-wave-c1 + W22b-frontend-wave-c2 SPLIT per F4 §3.6 trigger**(Option A+B Wave C scope ~42 days exceeds single phase;split decision at W22 kickoff post-W20 close)/ Wave D Tier 2 hold post-Beta Q12;W16 F1-F4 Track A IT cred parallel track,mock-auth default continues through Wave C per user 岔口 2)|

**累計**:**18 phase closed**(W1-W15 + W17 + W18 + W19;Tier 1 12-week sprint extended via W12-W15 UI sprint cycle pivot — architecture v5.1 → v6 amendment + 9 views — then W17 Beta-hardening parallel to W16, then W18 unified application shell IA per ADR-0024, then **W19-frontend-audit-and-adr-draft** plan phase for design-mockups → real-`frontend/` implementation with 6 ADRs Accepted(0025/0026/0027/0028/0029/0031)+ Wave C C1+C2 split trigger activated per Chris Option A+B picks);**W16 status:draft**(F5 backend stub closure cascade DONE;F1-F4 Track-A-blocked pending IT cred populate event + R-B1 closure — parallel-track to W17/W18/W19)— **W19-frontend-audit-and-adr-draft closure 2026-05-16**(phase Gate PASS WITH WAVE-C-SPLIT-TRIGGERED CAVEAT;6 ADRs Accepted via Chris pick;2 NEW dependencies triggered Key Vault SDK + Entra Graph SDK — H2 + R8 mitigation per ADR-0017 to land Wave C kickoff Plan B sequencing decisions)
...
**Last Updated**:2026-05-16(W19-frontend-audit-and-adr-draft phase **closed** — Gate PASS WITH WAVE-C-SPLIT-TRIGGERED CAVEAT;6 ADRs Accepted via Chris pick 4 strategic decisions:**Option A** ADR-0027 full RBAC + **Option B** ADR-0026 Settings fully editable + **Option B** ADR-0031 Conversation History server-side + **Option C** ADR-0029 `/kb/[id]/docs/[docId]` route — 3 of 4 picks went **maximum scope** over W19 F2 §6 minimum-recommend;**Wave C MUST split into C1+C2** per F4 §3.6 trigger;**2 NEW dependencies**(Key Vault SDK + Entra Graph SDK)triggered;**Wave A backend +3 days** per ADR-0031 Option B;§10 W19 row → closed verdict + W20+ candidates with Wave C C1+C2 split note + Wave A scope re-confirm at kickoff(7-tab vs 8-tab);Next NNNN advances 0025→0033 with 0030/0032 SKIPPED;5 commits this phase(`8bb380b` kickoff + `15aaca9` F1 + `d5bd196` F2 + `77941d1` F3 + `4f03fab` F4+F5 + this F6 closeout);Update history 加 row;CLAUDE.md unchanged — W19 是 plan-phase 而非 H1/H2/H3 改動;6 Accepted ADRs propose `architecture.md v6` amendments to land per W20+ phase kickoffs)
