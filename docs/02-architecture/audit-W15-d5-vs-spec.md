...
| **#3 C05 Context Expander missing** | 🚨 P0 | spec §3.1 mentions step;`prev_chunk_id` + `next_chunk_id` populated but never consumed | ✅ ADR-0020 Option A landed + backend module landed + 11 unit tests passed;**W16 F5.5 commit `1dbcdf3` further unblocked frontend Session 2** via `/debug/trace/{trace_id}` Langfuse SDK integration | 🚧 ADR-0020 Session 2 frontend V6 6→9 stage `PipelineStageCollapsible` expansion(backend `/debug/trace` endpoint NOW READY post W16 F5.5)|
