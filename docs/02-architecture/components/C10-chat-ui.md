...
> **CH-009 amendment(2026-06-08,Chat image relevance per ADR-0046)**:chat 圖片 display 層三項(`frontend/lib/chat/citation-images.ts` + `chat/page.tsx`)—— ① **Decorative filter(OD-1)**:`dedupeCitationImages` skip `isDecorativeImage`(`min(width,height)<DECORATIVE_MIN_PX=64`,dims 已知先判;0×0 legacy 保留),走手冊 Tip/Note 燈泡 icon(dims 由 C01 ADR-0046 probe 落 index)。② **Per-KB cap(OD-2)**:`MessageRow` cap = `maxImagesPerAnswer ?? INLINE_IMAGE_CAP`(`ChatThread`→`MessageRow` thread;chat page 讀 `activeKb.config.max_images_per_answer ?? null`)。③ **Document-order cap(OD-3,relevance-select reverted 2026-06-08)**:`selectInlineImages(deduped, cap)` = `deduped.slice(0, cap)`(純 document-order,Finding D)→ 章節概覽圖(§3.1.1 High Level Process)照手冊流程 lead;`dedupedImages.length`(全 deduped)仍驅動「View all in Image Library」。**原 relevance-select 把低分(score 0 expansion neighbour)概覽圖排出 cap,令 lead 變 mid-procedure step,對程序手冊錯方向,已 revert**。Live `/query` 驗:40 unique → 1 個 93×62 燈泡判 decorative、39 內容圖保留、dims 0 miss。
