...
- [ ] **AC4** — Unknown `kb_id` → **404** `{code: "kb.not_found", message: "kb_id={kb_id} not registered", actionable_hint: "Create the KB via POST /kb first."}`
  > **Reconciled by CH-002 (2026-05-12, Decision F6 = Option a)**: the as-built impl reuses the pre-existing shared `_verify_kb_or_404` helper, so the actual envelope code is `resource.not_found` (the global `_STATUS_TO_CODE[404]` map), not `kb.not_found` — with `message="KB '{kb_id}' not found"` and `actionable_hint="Verify the KB / document / chunk id and retry."`. This is **acceptable as-is**: the message already names the KB, and the frontend doesn't branch on `kb.not_found` vs `resource.not_found` (both map to the same "verify the id" UX). The `kb.not_found` text above was aspirational at draft time; no code change is made — CH-001 stays `done`. (CH-002 Option b — a dedicated `kb.not_found` code on the document routes — was considered and rejected: `_verify_kb_or_404` is shared by 4 routes and the marginal benefit didn't justify the diff.)
...
| 2026-05-12 | **AC4 reconciled by CH-002** (Decision F6 = Option a) — as-built impl uses the shared `_verify_kb_or_404` → envelope code is `resource.not_found`, not `kb.not_found`; documented as acceptable inline at §3 AC4; **no code change**, CH-001 stays `done`. (Doc reconcile only — not a behavioural deviation.) | Chris(2026-05-12 via CH-002) |
