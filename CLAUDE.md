...
| **加 / 改 UI 文案(i18n key / zh 譯文 / 術語)** | **[`frontend/messages/GLOSSARY.md`](./frontend/messages/GLOSSARY.md)**(術語權威 + §F 維護規則,W103 per ADR-0075) | 唔 hardcode 文案落 `.tsx`;en.json + zh.json 同步(缺一邊 = runtime throw);術語過 A / B / C 類 + 排版過 D 類;machine gate = `npx vitest run tests/unit/i18n-dictionaries.test.ts`;新譯法爭議 → 問用戶拍板補 E 類,唔可以靜靜引入第三種譯法 |
